...
An Android build compiles five backends: SDL3 GPU HAL, OpenGL ES 3.0 HAL, OpenGL ES 2.0 HAL,
Miniwin Paletted Software and Miniwin Emulation. **Game default** clears the setting and lets
the game pick, which is OpenGL ES 3.0 on anything that can run it.
...
this build supports but the current window could not offer, and a value the game no longer
...
not build - the game cannot fall back within that launch, because the OpenGL renderers cannot
initialize on the window it made. It stops with a message naming the renderer and offers
Settings, where choosing another renderer or Game default fixes the next launch.

## Render resolution

Render resolution is independent of the renderer. The game's own coordinates stay at 640x480
